...
class DeepSeekAssistant {
    ...
    async callDeepSeekAPI(userMessage, tableData, model = 'deepseek-chat') {
        ...
        const messages = [
            {
                role: "system",
                content: `你是金融产品分析Agent。仅按下列表头顺序输出"一行TSV"（制表符分隔），不得有任何解释/前后缀：

公司名称	参考价值	最后更新时间	应用赛道	细分场景	一句点评	成立时间	成立国家	发展阶段	业务模式	服务渠道	官网链接	业务简介	AI相关功能亮点	公开参考资料/链接	使用链接/途径	公司类别	最后更新人	开源社区项目
...
「应用赛道」与「细分场景」必须从下面选项中**单选**，细分场景只能取自所选赛道的子项；不得输出清单外词。 
...
细分场景映射（仅能在对应赛道里单选，用"、"分隔）： 
- 资产管理：投研助手；AI投资；智能投资助理
- 财富管理：个人财务规划；智能投资助理；理财师co-pilot；信用评级
- 投行：AI投行；投研助手；合规管理
- 保险：承保；核保；理赔
- 信贷：贷款审批；贷后催收；信用评级；反欺诈
- 内部运营：财务自动化；合规管理；流程优化；智能客服
- 支付：Agent支付；反欺诈
- Web3：Web3

若无法严格映射，细分场景填"-"。只输出一行TSV。`
            },
            ...this.conversationHistory,
            {
              role: "user",
